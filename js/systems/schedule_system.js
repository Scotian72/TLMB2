...
// === Training Camp Scrimmage Scheduling (additive) ===
(function(){
  'use strict';
  var TL = window.TL = window.TL || {};
  TL.Systems = TL.Systems || {};
  TL.Systems.Schedule = TL.Systems.Schedule || {};

  // Create simple same-org scrimmages across divisions for the given day
  TL.Systems.Schedule.createCampScrimmages = function(day){
    var GS = TL.GameState;
    var orgIds = Object.keys(GS.organizations || {});
    var games = [];
    orgIds.forEach(function(orgId){
      var org = GS.organizations[orgId];
      if(!org || !org.divisions) return;
      Object.keys(org.divisions).forEach(function(divKey){
        var div = org.divisions[divKey];
        // If we have at least 2 squads/teams, pair first two; otherwise mirror vs. ghost
        var teams = (div.teams && div.teams.length) ? div.teams.slice(0) : [];
        var home = teams[0] || { id: orgId+"-"+divKey+"-A" };
        var away = teams[1] || { id: orgId+"-"+divKey+"-B" };
        games.push({
          id: "CAMP-"+orgId+"-"+divKey+"-"+day,
          type: "scrimmage",
          day: day,
          orgId: orgId,
          division: divKey,
