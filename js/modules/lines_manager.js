...
// === Additive: minimal roster/lines ensure helpers for camp ===
(function(){
  'use strict';
  var TL = window.TL = window.TL || {};
  TL.Modules = TL.Modules || {};
  TL.Modules.Lines = TL.Modules.Lines || {};

  TL.Modules.Lines.ensureRoster = function(orgId, division, teamId){
    var GS = TL.GameState;
    GS.rosters = GS.rosters || {};
    GS.rosters[teamId] = GS.rosters[teamId] || [];
    // If empty, auto-generate placeholders using PlayerGenerator (if present) or simple stubs
    if (GS.rosters[teamId].length < 10) {
      var need = 10 - GS.rosters[teamId].length;
      for (var i=0; i<need; i++){
        var pid = "P-"+teamId+"-"+(Date.now()%100000)+"-"+i;
        GS.players = GS.players || {};
        GS.players[pid] = GS.players[pid] || {
          id: pid, orgId: orgId, division: division, teamId: teamId,
          name: "Camp Player " + (GS.rosters[teamId].length + 1),
          overall: 30 + Math.floor(Math.random()*20),
          pos: (i%5===0?'G':'R') // simple goalie distribution
        };
        GS.rosters[teamId].push(pid);
      }
    }
  };

  TL.Modules.Lines.ensureCampLines = function(teamId){
    var GS = TL.GameState;
    GS.lines = GS.lines || {};
    GS.lines[teamId] = GS.lines[teamId] || {
      offense: [], defense: [], goalie: null
    };
    var roster = (GS.rosters && GS.rosters[teamId]) || [];
    // Simple auto-lines: first goalie as goalie, next 5 runners offense, next 5 defense
    var runners = roster.filter(function(pid){ return (GS.players[pid]||{}).pos !== 'G'; });
    var goalies = roster.filter(function(pid){ return (GS.players[pid]||{}).pos === 'G'; });
    GS.lines[teamId].goalie = goalies[0] || runners[0] || roster[0] || null;
    GS.lines[teamId].offense = runners.slice(0,5);
    GS.lines[teamId].defense = runners.slice(5,10);
  };
})();
